    ...
    handleSubmit (e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          const hide = this.$message.loading('正在登陆..', 0)
          getAccessToken(params).then(res => {
                setTimeout(hide, 2500)
                this.$notification['success']({
                message: '提示',
                description: '你的access token：' + res.access,
                duration: 2
                })
                storage.set(AUTHORIZATION, 'Bearer' + ' ' + res.access, 7 * 24 * 60 * 60 * 1000)
                sessionStorage.setItem(AUTHORIZATION, 'Bearer' + ' ' + res.access, 7 * 24 * 60 * 60 * 1000)
                console.log('bearer', res.access)
                this.$router.push('/')
